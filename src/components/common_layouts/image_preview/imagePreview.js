...
import { AcLightBox } from '../../UiComponents/AcImports';
...
export class ImagePreviewComponent extends React.Component{
        ...
        if(id){
            submitSets(submitCollection.findProdImageData, ('?productId='+id), true).then(res => {
                ...
                if(res && res.status){
                    ...
                    alertService.error(i18n.t("ERROR_OCCURRED"));
    ...
    handleImageLoadError = (e) =>{
        ...
        alertService.error(i18n.t("COULD_NOT_THE_LOAD_IMAGE"));
    }
    ...
    render(){
        return(this.props.hideheaderlables?<>
            {this.props.isshow?<AcLightBox 
                showcount={false} allowZoom={true} allowReset={true}
                datalist={this.state.cobj.imageUrl?[this.state.cobj.imageUrl]:[]}
                onClose={this.handleImgClose}
                />:<></>}
        </>:<>
            <Modal className="img-preview-modal" show={this.props.isshow} dir={this.props.isRTL} centered={true} onHide={() => this.props.handlePreviewModal(null,false)} animation={false} backdrop="static">
                <Modal.Body className="text-center">
                    ...
                    <Col className={this.state.isloaded === true ? "" :"d-none"}>
...

                        {
                            ...
                            <div className='loading-anim-div' style={{paddingTop:"16%"}}>
                                <div className="animewrapper animation-2 col-centered">
                                    <div className="shape shape1"></div>
                                    <div className="shape shape2"></div>
                                    <div className="shape shape3"></div>
