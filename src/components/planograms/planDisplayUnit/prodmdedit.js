...
import { Accordion, Alert, Button, Col, Form, Modal, Row, Table } from "react-bootstrap";
import { ArrowSwitchIcon, ChevronDownIcon, ChevronUpIcon } from '@primer/octicons-react';
...
import { restrictDecimalPoint } from "../../../_services/common.service";
...
export default function ProdMDModal(props) {
    ...
    const [collapseOpened, setCollapseOpen] = useState(false);
    ...
    const handlevalchange = (evt,type) => {
      ...
      if(evt.target.value && evt.target.value >= 0){
        ...
        if(!restrictDecimalPoint(evt.target.value,3)){
    ...
    const checkchange = (obj) => {
      ...
      if(props.saveobj){
        for (let y = 0; y < props.saveobj.fieldsList.length; y++) {
          const fieldobj = props.saveobj.fieldsList[y];
          
          for (var i = 0; i < fieldobj.planogramShelfDto.length; i++) {
            const shelveitem = fieldobj.planogramShelfDto[i];
      ...
      if(cvprodobj){
        confirmAlert({
            ...
            buttons: [{
                ...
                onClick: () => {
                  submitSets(submitCollection.updateProdDimention, cvprodobj, true, null, true).then(resp => {
                      //console.log(res);
                      if(resp && resp.status){
    ...
    const accordionChange = (evt) => {
      setCollapseOpen((evt !== null));
    } 
    ...
    return (<Modal show={props.pemshow} className="pgram-editproduct" animation={false} onHide={toggleClose} backdrop="static" keyboard={false} dir={props.isRTL}>
      <Modal.Header>
        <Modal.Title style={{fontWeight:"800",color:"#5128a0"}}>
          <small>{cvprodobj?cvprodobj.productName:"-"}</small>
          <label className="uom-label">{t("uom")} <div>{cvprodobj?cvprodobj.uom:"-"}</div></label>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Row>
          ...
          <Col>
            <Form.Group>
              ...
              <Button type="button" variant="outline-danger" size="sm" onClick={() => handleFlip(2)} style={{width:"100%",marginTop:"10px",borderRadius:"15px",fontWeight:"700"}}>H <ArrowSwitchIcon size={14}/> D </Button>
            </Form.Group>  
          </Col> 
          <Col>
            <Form.Group>
              <Form.Label style={{fontWeight:"600",color:"#5128a0",fontSize:"14px",textTransform:"uppercase"}}>{t('depth')}</Form.Label>
              <Form.Control type="number" value={cvprodobj?cvprodobj.depth:0} onChange={e => handlevalchange(e,"depth")} style={{fontWeight:"600"}} />
              <Button type="button" variant="outline-danger" size="sm" onClick={() => handleFlip(3)} style={{width:"100%",marginTop:"10px",borderRadius:"15px",fontWeight:"700"}}>W <ArrowSwitchIcon size={14}/> D </Button>
            </Form.Group>  
          </Col> 
        </Row>
        <Col>
          {loadingpgrams?<Col className="text-center prodedit-loading"><img src={loadinggif} alt="loading gif" /></Col>:<></>}
          {isshowwarning?<Alert variant="warning" style={{position:"relative"}}>
            {t('THIS_NOT_EFFECT_ELIER_PLANO_ONLY_FOR_THIS_AND_FUTURE_PLANO')}
            <Accordion>
              <Accordion.Item eventKey="0">
                <Accordion.Header style={{cursor:"pointer"}}>{t('VIEW_USED_PLANOGRAMS')} {collapseOpened?<ChevronUpIcon size={16} />:<ChevronDownIcon size={16} />}</Accordion.Header>
                <Accordion.Body>
                  <Col style={{padding:"5px",maxHeight:"150px",overflowY:"auto"}}>
                    <Table>
                      <tbody>
