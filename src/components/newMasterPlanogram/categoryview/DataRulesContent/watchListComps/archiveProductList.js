...
class ArchiveProductsList extends React.Component {
    constructor(props){
        super(props);
    ...
    loadMainList = (sobj)=>{
        ...
        submitSets(submitCollection.vmpGetDepartmentArchivedProducts, sobj, true).then(res => {
    ...
    restore = (pobj, index) =>{
        confirmAlert({
            ...
            message: this.props.t("NEW_PROD_RESTORE_CONFIRM"),
            ...
            buttons: [{
                ...
                onClick: () => {
                    ...
                    submitSets(submitCollection.restoreProduct, sobj, true, null, true).then(resp => {
                                ...
                                return (
                                    <Col key={index}>
                                        {item.isShow===true ?
                                            <ListGroup.Item >
                                                <Row>
                                                    <Col xs={10} className="details-col products" style={{paddingTop:"5px", paddingBottom:"2px"}}>
                                                        <div className='barcode'><CopyToClipboard text={item.barcode} onCopy={() => this.props.copyToClipboard()}><b>{item.barcode}</b></CopyToClipboard></div>
                                                    ...
                                                    <Col xs={2} className='btns-col' style={{paddingTop:"10px"}}>
                                                        <OverlayTrigger placement="top" overlay={<Tooltip className={"resolve-content-item-tooltip "}><label>{this.props.t("btnnames.restore")}</label></Tooltip>}><FeatherIcon onClick={()=>this.restore(item, index)} icon="rotate-ccw" size="12"/></OverlayTrigger>
...
export default withTranslation()(withRouter((ArchiveProductsList)));