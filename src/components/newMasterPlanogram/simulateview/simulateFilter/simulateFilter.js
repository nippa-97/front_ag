...
import './simulateFilter.css';
import { XIcon } from '@primer/octicons-react';

export class SimulateFilter extends Component {
    constructor(props){
        super(props);
    ...
    render(){
        return(
            <Row className="simulation-filter-row">
                <Col xs={12} className={"simulation-filter-section " +(this.props.isRTL==="rtl"?"RTL":"")}>
                    <ul className="topbtn-list list-inline">
                        ...
                        <li className={"list-inline-item "+(this.props.isRTL==="rtl" ? "float-left" :"float-right")}>
                            <Button variant="danger" onClick={()=>this.props.filterSimulation(this.state.simulateFilterObj)} size="sm" style={{textTransform:"uppercase"}}>{this.props.t("Simulate")}</Button>
                        </li>
                    </ul>
...
export default withTranslation()(withRouter(connect(null)(SimulateFilter)));
