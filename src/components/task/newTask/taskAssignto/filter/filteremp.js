...
class Filteremp extends Component {
    ...
    loadfiltertypes = () => {
        ...
        if (this.props.roleIds.length > 0) {
            ...
            this.setState({ frolelist: rglist }, () => {
    ...
    handleRoles = (obj) => {
        ...
        this.setState({ frolelist: list }, () => {
            // console.log(this.state.frolelist);
        })
    ...
    removeAll=(type)=>{
        if(type==="region"){
            var regionlist = JSON.parse(JSON.stringify(this.state.fregionlist))
            for (let i = 0; i < regionlist.length; i++) {
                const element = regionlist[i];
                    element.isSelected = false
            }
            this.setState({ fregionlist: regionlist })
        }
        if(type==="store"){
            var storelist = JSON.parse(JSON.stringify(this.state.fstorelist))
            for (let i = 0; i < storelist.length; i++) {
                const element = storelist[i];
                    element.isSelected = false
            }
            this.setState({ fstorelist: storelist })
        }
        if(type==="role"){
            var rolelist = JSON.parse(JSON.stringify(this.state.frolelist))
        ...
        if(type==="group"){
            var grouplist = JSON.parse(JSON.stringify(this.state.fgrouplist))
            for (let i = 0; i < grouplist.length; i++) {
                const element = grouplist[i];
                    element.isSelected = false
            }
            this.setState({ fgrouplist: grouplist })
        }
    ...
    render() {
        return (
            <div className="Assignfilter">
                ...
                <Col className="filterbody">
                    <Col>
                        ...
                        >{this.props.t('region')}</Button>
                          <Button className="addallfilter" onClick={(e)=>this.removeAll("region")}>{this.props.t("REMOVE_ALL")}</Button>
                        <Button className="addallfilter" onClick={(e)=>this.addAll("region")}>{this.props.t("ADD_ALL")}</Button>
                    ...
                    <Col>
                        ...
                        <Button className="addallfilter" onClick={(e)=>this.removeAll("store")}>{this.props.t("REMOVE_ALL")}</Button>
                         <Button className="addallfilter" onClick={(e)=>this.addAll("store")}>{this.props.t("ADD_ALL")}</Button>
                    ...
                    <Col>
                        ...
                        >{this.props.t('ROLE')}</Button>
                         <Button className="addallfilter" onClick={(e)=>this.removeAll("role")}>{this.props.t("REMOVE_ALL")}</Button>
                         <Button className="addallfilter" onClick={(e)=>this.addAll("role")}>{this.props.t("ADD_ALL")}</Button>
                                ...
                                <Row>
                                    {this.state.frolelist && this.filterfunction("all").map((role, i) =>
                                        <Col md={4} className="filterinlist" key={i}>
                                            ...
                                            <TooltipWrapper text={role.name}><span className='taskadd-labeltxt'>{role.name}</span></TooltipWrapper>
                                        </Col>
                                    )}
                                </Row>
                    ...
                    <Col>
                        ...
                        >{this.props.t('GROUP')}</Button>
                         <Button className="addallfilter" onClick={(e)=>this.removeAll("group")}>{this.props.t("REMOVE_ALL")}</Button>
                         <Button className="addallfilter" onClick={(e)=>this.addAll("group")}>{this.props.t("ADD_ALL")}</Button>
                                ...
                                <Row>
                                    {this.state.fgrouplist && this.filterfunction("group").map((groups, i) =>
                                        <Col md={4} className="filterinlist" key={i}>
                                            ...
                                            <TooltipWrapper text={groups.groupName}><span className='taskadd-labeltxt'>{groups.groupName}</span></TooltipWrapper>
                                        </Col>
                                    )}
                                </Row>
                ...
                <Col><Button className="applybtnpink" onClick={() => this.applyfilterhandle()}>{this.props.t('APPLY')}</Button></Col>
            </div >
        );
    }
}
...
export default withTranslation()(withRouter(Filteremp));