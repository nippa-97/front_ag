    ...
    render() {
        return (
            <Col className="fromlist">
                <div className="title" style={{ color: this.props.errors.listempty && "red" }}>{this.props.t('START_FILLING_OUT_THE_CHECKLIST')}</div>
                {this.props.checkList.map((type, i) =>
                    <React.Fragment key={i}>
                        {!type.isDelete ? <div key={i} className="list long-txt"  >
                            <FeatherIcon icon="check-circle" size={14} style={{ color: (this.props.errors.listitemempty && type.optiontext === "") && "red" }} />
                            ...
                            {/* <input type="text" name="name" style={{ borderBottom: this.props.errors.listitemempty && "1px solid red" }} placeholder={this.props.t("TYPE_FIRSTACTION_HERE")} value={type.optiontext} onChange={(e) => this.handleOptiontext(e, i)} /> */}
