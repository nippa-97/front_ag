...
class Approverfilteremplist extends Component {
    ...
    handleactvieEmp = (emp) => {
        var active = false
        var allocationlist = this.props.sobj.taskApproversDtoList
        for (let i = 0; i < allocationlist.length; i++) {
            const allocator = allocationlist[i];
            if (!allocator.isDelete&&allocator.approverUuid === emp.userUUID) {
                active = true
                break
            }
        }
...
export default withTranslation()(withRouter(Approverfilteremplist));